// src/main/scala/milestoneProject/HttpClient/HttpClient.scala
package httpclient
...
object HttpClient{
  ...
  trait API extends HttpClient {
    ...
    def executeHttpPost(url: String, body: Map[String,String]): HttpResponse = {
      ...
      if (response.getStatusCode != 200) sys.error(s"Failed to Post to: $url")
    ...
    def executeHttpGet(url: String): HttpResponse = {
      ...
      val response = request.execute().get()
      asyncHttpClient.close()
      if (response.getStatusCode != 200) sys.error(s"Failed to Get: $url")
      else formatResponse(response)
    }
